                ...
                <div className="flex gap-2 items-center">
                    <div className="flex gap-2">
                        {["Male", "Female"].map((gender) => {
                            return (
                                <button
                                    className={`flex justify-center items-center gap-1 font-medium ${selectedgender == gender ? "text-horizon" : ""}`}
                                    onClick={() => {
                                    ...
                                    <div
                                        className={`h-3 w-3 border rounded-full ${selectedgender == gender ? "bg-horizon border-transparent" : ""}`}></div>
                                    {gender}
                                </button>
                            );
                        })}
...
function Booking() {
    ...
    const [passengerData, setPassengerData] = useState(Array.from({ length: passengerCount }, () => ({ title: "" })));
        ...
        <div id="cont" className="pt-14">
            ...
            <div id="render" className="flex flex-col justify-center items-center">
                {Array.from({ length: passengerCount }, (_, i) => (
                    <RenderPassenger key={i} index={i + 1} data={passengerData[i]} onChange={(field, value) => updatePassenger(i, field, value)} />
                ))}
            </div>
